...
import { History, KeyRound, Image, Database, Loader2 } from "lucide-react";
...
export function CodexAuthSettings({
  ...
  const handleCleanConfirm = async () => {
    ...
    setIsCleaning(true);
    try {
      const freedBytes = await settingsApi.cleanCodexDatabase();
      const mb = (freedBytes / 1024 / 1024).toFixed(2);
      toast.success(t("settings.codexCleanSuccess", { mb }));
    } catch (e) {
      console.error("Failed to clean codex database:", e);
      toast.error(t("settings.codexCleanFailed"));
    } finally {
      setIsCleaning(false);
    }
  };
  ...
  return (
    <section className="space-y-4">
      ...
      <ConfirmDialog
        ...
        title={t("confirm.codexClean.title")}
        message={t("confirm.codexClean.message")}
        confirmText={t("confirm.codexClean.confirm")}
